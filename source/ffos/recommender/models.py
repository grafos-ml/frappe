...
class Matrix(models.TextField):
    ...
    def to_python(self, value):
        if isinstance(value,basestring):
            ...
            return np.frombuffer(base64.decodestring(value),
                dtype=np.float64)
        return value
...
class TensorModel(models.Model):
    ...
    @staticmethod
    def train():
        reader = MySQLDataReader(settings.DATABASES['default']['HOST'],
            3306,settings.DATABASES['default']['TEST_NAME' if settings.TESTING \
                else 'NAME'],
            settings.DATABASES['default']['USER'],
            settings.DATABASES['default']['PASSWORD'])
        tensor = JavaTensorCoFi(20,5,0.05,40,
            [len(FFOSUser.objects.all()),len(FFOSApp.objects.all())])
        ...
        TensorModel.objects.create(matrix=users,rows=users.shape[0],
            columns=users.shape[1],dim=0)
        TensorModel.objects.create(matrix=items,rows=items.shape[0],
            columns=items.shape[1],dim=1)
        return users,items
