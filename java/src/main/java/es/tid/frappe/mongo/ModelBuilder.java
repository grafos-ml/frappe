...
import es.tid.frappe.recsys.PopPredictor;
import es.tid.frappe.recsys.Predictor;
import es.tid.frappe.recsys.RandomPredictor;
...
public class ModelBuilder {
	...
	DataReader reader;
	Hashtable<String, DataWriter> writers = new Hashtable<String, DataWriter>();
	Hashtable<String, Predictor> predictors= new Hashtable<String, Predictor>();
	...
	public static SimpleJSAP getProgrammOptions() throws JSAPException {
		...
		optAlpha.setHelp("The alpha controls the importance of observation vs non observed values. Good numer is [5, 100].");
		...
		FlaggedOption optMongoDBname = new FlaggedOption("dbname")
			.setStringParser(JSAP.STRING_PARSER).setDefault("frappe")
		...
		SimpleJSAP jsap = new SimpleJSAP(ModelBuilder.class.getName(),
				"Learns user, item, context1, context2, rating model (latent representation).",
				new Parameter[] { optIter, optDim, optLambda, optAlpha, optPredictor,
									optMongoHost, optMongoPort, optMongoDBname, optMongoUser, optMongoPswd });
	...
	public static void main(String[] args) throws JSAPException, UnknownHostException {
		...
		modelBuilder.setReader(new MongoDataReader(
		...
		modelBuilder.setWriter(new MongoDataWriter(
				jsapResult.getString("host"),
				jsapResult.getInt("port"),
				jsapResult.getString("dbname"),
				jsapResult.getString("user"),
		...
		if (jsapResult.getString("predictor").equals("TF")){
			TensorCoFi tf = new TensorCoFi(
					jsapResult.getInt("dim"), 
					jsapResult.getInt("nIter"),
					jsapResult.getFloat("lambda"), 
